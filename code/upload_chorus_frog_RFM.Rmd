...
Create subset of files in CSV & in Site_41.
```{r}
...
MFCCFunction Truncates filename in `Class` after an underscore.
```{r}
...
Cluster with PCA and also add new column in feature data frame with new column containing "absent" or "present" depending on classification associated with "Class" (the WAV filename) in Pfer_clean.
