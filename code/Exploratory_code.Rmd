...
##INSERT CAROLINE CODE HERE Try reading in duke forest data. Need to read in the wav files, categorize as present or absent using the csv, and select out a small portion to do stuff with (so the computer doesn't explode)

```{r}

vars <- read.csv("~/Library/CloudStorage/Box-Box/innovations-research-tech/PAM/DukeForestPAMdata/DukeForest/Sites/Site12/DFexportAll.csv") #this tells me which ones are present/absent
...
MyFeatureDataFrame <- MyFeatureDataFrame %>% 
  mutate(class = if_else(str_starts(MyFeatureDataFrame$Class, "absent"), "absent", "present"))
...
ml.model.svm <- e1071::svm(MyFeatureDataFrame[, 2:ncol(MyFeatureDataFrame)], MyFeatureDataFrame$class, kernel = "radial", 
                           cross = 3,
                           probability = TRUE)
...
```{r}
set.seed(234)
trees_folds <- vfold_cv(train,v = 2) #usually you use something like 10-fold cross validation 
```
