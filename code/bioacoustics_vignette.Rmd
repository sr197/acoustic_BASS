---
title: "adapted code from: Detection, Extraction and Classification of Bird and Bat Vocalizations in R"
author: "Sarah Roberts, adapted from code by Francois Fabianek, Jean Marchal"
...
Now that we have recordings stored in the data directory, we can read one of them to look at its structure and content. Let's use the `read_audio()` function in **bioacoustics** to manually read in one of Caroline's recordings:

```{r read_audio, eval=FALSE}
WaveFile <- read_audio("~/Library/CloudStorage/Box-Box/innovations-research-tech/PAM/acoustic_BASS/data/subset/present1.wav")
WaveFile
```
...
Now that we have explored a WAV file, we will use the Fast Fourrier Transform (FFT) to compute a frequency-time representation of the recording, called a spectrogram. A spectrogram being the representation of the spectrum of frequencies in a recording as they vary through time. This representation, although not optimal, is still commonly used to detect animal vocalizations and extract acoustic features useful for classification with the purpose of animal identification. \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
spectro(WaveFile, tlim = temp_slice, FFT_size = 512, ticks_y = ticks)
```
...
spec_mx <- fspec(WaveFile, tlim = temp_slice, FFT_size = 512, rotate = TRUE)
...
spec_o <- fspec(WaveFile, tlim = temp_slice, flim = freq_slice, FFT_size = 512, rotate = TRUE)
...
spec_f <- fspec(WaveFile, tlim = temp_slice, flim = freq_slice, FFT_size = 512, rotate = TRUE)
...
Let's start with the `threshold_detection()` function on a recording containing calls from one of Caroline's primates. This function is an amplitude threshold detector that picks up audio events above the Signal to Noise Ratio (SNR). It combines several algorithms for detection, filtering and audio feature extraction. We will play with the arguments of this function to understand their implication in the detection and extraction of audio events (here, calls of some primate).
...
TD <- threshold_detection(
  WaveFile, # Either a path to an audio file (see ?read_audio), or a Wave object
...
TD <- threshold_detection(
  WaveFile, threshold = 12, time_exp = 1, min_dur = 140, max_dur = 440, 
...
c(TD$data$event_start[[5]], TD$data$event_end[[5]]) / slot(WaveFile, "samp.rate")
```
...
# Compute the number of extracted WaveFile calls
sum(startsWith(Event_data$filename, "Cat"))

# Add a "Class" column: "WaveFile" vs. other species of birds "OTHERS"
classes <- as.factor(ifelse(startsWith(Event_data$filename, "Cat"), "WaveFile", "OTHERS"))
...
We will use Keras in R which requires to install several packages in [Python](https://www.python.org/downloads/) Guidelines to install Keras properly in R are available [here](https://keras.rstudio.com/)
...
Francois: [francois.fabianek\@wavx.ca](mailto:francois.fabianek@wavx.ca){.email}\
Jean: [jean.marchal\@wavx.ca](mailto:jean.marchal@wavx.ca){.email}\
